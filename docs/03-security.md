...
| M7 | Medium | `.github/workflows/docker-publish-axum.yml:34` | `actions/checkout@v3` unpinned and EOL (Node 16 runtime). All other third-party actions are correctly SHA-pinned — inconsistent policy, supply-chain risk. | Pin `actions/checkout` v4 by full SHA; add Dependabot for docker + github-actions. |
...
| # | Issue | Sev |
|---|-------|-----|
| 1 | Runtime as root, full toolchain image (`rust:1.71-slim`, EOL), no `USER` | High (H3) |
| 2 | No minimal runtime stage; binary is dynamically linked (glibc) | High (H3) |
| 3 | Base image not digest-pinned; moving tag; EOL toolchain | Medium |
| 4 | `COPY ./jwt-lib/ ../` — copies into `/`, works only via path-dep resolution quirk | Low (L4) |
...
Good practices present (verified): 5 of 6 third-party actions pinned to full commit SHAs; no `pull_request_target`/`workflow_run`; the only `run:` step uses env-var indirection for cosign signing; `id-token: write` scoped for sigstore.

Issues: M7 (`actions/checkout@v3` unpinned, EOL); no `cargo audit` or `--locked` build; no Dependabot; no `concurrency:`/`timeout-minutes`; `cosign-release: v2.1.1` stale; H5 — no `JWT_SECRET` handling anywhere: the pipeline publishes the image containing the hardcoded key by design.
...
12. **M7** — Pin `actions/checkout@v4` SHA; Dependabot for actions and cargo; `concurrency` + `timeout-minutes`.
